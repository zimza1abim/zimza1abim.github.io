...
mousepad inputs.conf
[monitor:///var/log/apache2/access.log] # 로그 수집 path
disabled = false
index = index
sourcetype = access_apache

mousepad outputs.conf
[tcpout]
defaultGroup = my_splunk
[tcpout:my_splunk]
Server = 192.168.40.135:9997 # kali 아이피 주소, splunk port 입력
```
...
# 칼리에서
curl http://192.168.40.134(우분투 ip) 접속 기록 남기기
